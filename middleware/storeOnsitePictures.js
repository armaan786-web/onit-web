...
const upload = multer({
    storage: multerS3({
        ...
        key: function (req, file, cb) {
            let fileFolderPath = commonKeys.s3FileFolderPathIdentifiers["TICKET_ONSITE_PICTURES"];
            let ticket_obj_id = req.params.ticket_obj_id;

            cb(null, `${fileFolderPath}/${ticket_obj_id}/${file.fieldname}-${Date.now()}.jpg`);
        }
    })
});
